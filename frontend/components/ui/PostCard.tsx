...
export function PostCard({ post }: { post: Post }) {
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-base-300 bg-base-100">
      {post.image && (
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-4 flex-grow leading-relaxed text-secondary">{post.caption}</p>

        <div className="mt-6 flex items-center gap-2 border-t border-base-300 pt-4 text-sm">
          <span className="font-semibold text-neutral">{userName || 'Loading…'}</span>
